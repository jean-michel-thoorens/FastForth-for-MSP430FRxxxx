...
\ ------------------------------\
    ...
    BIT.B #SMMSDA,&I2CSMM_IN    \ _     get SDA
...
\    MOV #6,T                        \ I2C_MR_DC_Wait_Start_Loop = 8 탎 @ 24 MHz
    BEGIN
        BIT.B #SMMSCL,&I2CSMM_IN    \ 3 P1DIR.3 SCL high ? 
        0= ?GOTO BW2                \ 2
        BIT.B #SMMSDA,&I2CSMM_IN    \ 3 P1IN.2 SDA high ?
        0= ?GOTO BW3                \ 2
        SUB #1,T                    \ 1
...
\       ----------------------------\
        0= IF                       \ 2     if SDA low
            BIT.B #SMMSDA,&I2CSMM_DIR   \ 3
            0= ?GOTO BW1            \ 2     collision : IN(MMSDA=0) XOR DIR(MMSDA=0) = 0
        THEN                        \
        SUB #1,W                    \ 3     dec count of bits
    0= UNTIL                        \ 2  
